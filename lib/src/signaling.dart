...
class Signaling {
  ...
  void startcall(String sessionId, String peerId, bool audio, bool video, bool localaudio, bool localvideo, bool datachannel,
      String mode, String source, String user, String password) {
  ...
  Future<void> _handleOfferMessage(Map<String, dynamic> data) async {
    try {
      ...
      var newSession = await _createSession(session,
          peerId: peerId, sessionId: sessionId, audio: useAudio.enabled, video: useVideo.enabled, dataChannel: useDataChannel);
      ...
      await newSession.pc?.setRemoteDescription(RTCSessionDescription(sdp, "offer"));
  ...
  Future<MediaStream?> createLocalStream(bool audio, bool datachannel) async {
    Map<String, dynamic> mediaConstraints = {'audio': _localAudio, 'video': false};
  ...
  Future<Session> _createSession(Session? session,
    ...
    if (_onlyDatachannel == false && (_localAudio == true || _localVideo == true)) {
  ...
  Future<void> _createOffer(Session session, String mode, String source) async {
    try {
      ...
      RTCSessionDescription s = await session.pc!.createOffer(_onlyDatachannel ? {} : dcConstraints);
  ...
  Future<void> _createAnswer(Session session) async {
    try {
      ...
      RTCSessionDescription s = await session.pc!.createAnswer(_onlyDatachannel ? {} : dcConstraints);
      await session.pc!.setLocalDescription(s);
      var delay = RandomString.currentTimeMillis() - _startTime;
