...
enum RecordOfferState {
  Redording,
  RecordClosed,
}

class RealOfferVideo extends StatefulWidget {
  static String tag = 'Offer_sample';
  final String peerId;
  final String selfId;
  ...
  @override
  _RealOfferVideoState createState() => _RealOfferVideoState();
}

class _RealOfferVideoState extends State<RealOfferVideo> {
  String? _selfId;
  String? _peerId;
          ...
          if (compare(sessionId, _SessionId) == 0) {
            ...
            if (state != null) {
              if (compare(state, "online") == 0) {
                ...
                if (_usedatachannel == true) {
                  _createDataChannel();
                }
                _createOffer(_Mode,_Source);
  ...
  onDataChannelTxtMessage(message) async {
    print('ondatachennel Message recv len =${message.length}');
...
*/
  ...
  _send_datachennel_msg_ex() {
    if (_dataChannelOpened) {
      var request = Map();
      request["title"] = "config_get";
      request["sequence"] = 0;
      request["channel"] = -1;
      request["payload"] = {};
      print("datachennel :_send_datachennel_msg_ex");
      dc?.send(RTCDataChannelMessage(_encoder.convert(request)));
    ...
    if (_inCalling == true) {
      ...
      for (var i = 0; i < senders.length; i++) {
        ...
        if (sender.track != null && sender.track!.kind == "audio") {
          ...
          //sender.track!.enabled = _mute;
          print(
              'muteLocalStreamSession track ------------------------: ${sender.track}');
        }
      }
    }
  }
  ...
  */
  ...
  Future<MediaStream> createLocalStream(
    ...
    if (audio == false && video == false && datachennel == true) {
      ...
      mediaConstraints = {
        'audio': { 
            'mandatory': [{'sampleRate': 0},{'channelCount': 0 },{'audio': false }] ,  
            'optional': [], 
        },
        'video': _localvideo
            ? {
  ...
  Future<void> _createDataChannel() async {
    ...
    RTCDataChannel channel =
        await pc!.createDataChannel("datachannel", dataChannelDict);
     ...
     try {
       ...
       LogUtil.v("sendOfferNoTrickle ${s!.sdp}");
            ...
            websocket_send('__offer', {
              ...
              "user": _User,
              "pwd": _Password,
              "iceservers": _encoder.convert(iceServers)
            });
    } catch (e) {
          print(e.toString());
    }
  ...
  Future<void> _createOffer(String mode, String source) async {
    ...
    try {
      ...
      RTCSessionDescription s =
          await pc!.createOffer(_onlydatachnannel ? {} : dcConstraints);
  ...
  Future<void> _createAnswer() async {
    try {
      ...
      RTCSessionDescription s =
          await pc!.createAnswer(_onlydatachnannel ? {} : dcConstraints);
      await pc!.setLocalDescription(s);
      _can_add_candidate = true;
      var delay = currentTimeMillis() - _start_time_;
      print(
  ...
  Future<void> captureFrame() async {
    try {
      ...
      print('captureFrame appPath: ' + appDocPath);print('captureFrame appPath: ' + appDocPath);
              ...
              if (_remotevideotrack!.id == receive.track!.id) {
                if (startrecorded == false) {
                  ...
                  if (_isWeb()) {
                    ...
                    _mediarecoder.start(2,recordFilepath, peerconnectid,
                        videoTrack: receive.track, audioChannel: audiochannel);
